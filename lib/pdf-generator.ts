...
export const generatePDF = async (data: PDFData): Promise<void> => {
  try {
    console.log("Starting PDF generation with direct download method")
    // استخدام وظيفة جديدة لإنشاء PDF وتنزيله مباشرة
    await generatePDFWithDirectDownload(data)
  } catch (error) {
...
// وظيفة جديدة لإنشاء PDF وتنزيله مباشرة
export const generatePDFWithDirectDownload = async (data: PDFData): Promise<void> => {
  try {
    ...
    try {
      ...
      // تنزيل الملف مباشرة بدلاً من فتح نافذة جديدة
      pdf.save(fileName)
    } catch (error) {
      console.error("Error generating PDF with direct download:", error)

      // تنظيف العنصر المؤقت
      document.body.removeChild(container)

      // إعادة محاولة باستخدام طريقة بديلة
      await generatePDFWithFallback(data)
    } finally {
      // تنظيف العنصر المؤقت
      document.body.removeChild(container)
    }
  } catch (error) {
    console.error("Error in PDF generation with direct download:", error)
